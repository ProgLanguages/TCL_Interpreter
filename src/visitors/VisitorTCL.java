...
##############################################################################*/
    ...
    public T visitExp_or(Exp_orContext ctx) {
        ...
        if (ctx.exp_or() != null) {
            ...
            if (var1.getTipo() == Constants.INT && var2.getTipo() == Constants.INT) {
                ...
                int line = (var1.getTipo() != Constants.STRING) ? ctx.exp_or().getStart().getLine() : ctx.exp_and().getStart().getLine(),
                        col = (var1.getTipo() != Constants.STRING) ? ctx.exp_or().getStart().getCharPositionInLine() : ctx.exp_and().getStart().getCharPositionInLine();
                Error.printError(msj, line, col);
            }
            return (T) var3;
        }
    ...
    public T visitExp_and(Exp_andContext ctx) {
        ...
        if (ctx.exp_and() != null) {
            ...
            if (var1.getTipo() == Constants.INT && var2.getTipo() == Constants.INT) {
                ...
                String msj = Error.incompatibleData(Error.ERR_STRING, tipo);
                int line = (var1.getTipo() != Constants.STRING) ? ctx.exp_and().getStart().getLine() : ctx.exp_ig().getStart().getLine(),
                        col = (var1.getTipo() != Constants.STRING) ? ctx.exp_and().getStart().getCharPositionInLine() : ctx.exp_ig().getStart().getCharPositionInLine();
                Error.printError(msj, line, col);
            }
            return (T) var3;
        }
        ...
        if (ctx.exp_ig() != null) {
            ...
            if (op.equals("ne") || op.equals("eq")) {
                if (var1.getTipo() == Constants.STRING && var2.getTipo() == Constants.STRING) {
                    ...
                    String msj = Error.incompatibleData(Error.ERR_STRING, tipo);
                    int line = (var1.getTipo() != Constants.STRING) ? ctx.exp_ig().getStart().getLine() : ctx.exp_rel().getStart().getLine(),
                            col = (var1.getTipo() != Constants.STRING) ? ctx.exp_ig().getStart().getCharPositionInLine() : ctx.exp_rel().getStart().getCharPositionInLine();
                    Error.printError(msj, line, col);
                }
            } else {
                String msj = null;
    ...
    public T visitExp_add(Exp_addContext ctx) {
        ...
        if (ctx.exp_add() != null) {
            ...
            if (msj != null) {
                int line = (var1.getTipo() == Constants.STRING) ? ctx.exp_add().getStart().getLine() : ctx.exp_mul().getStart().getLine(),
                        col = (var1.getTipo() == Constants.STRING) ? ctx.exp_add().getStart().getCharPositionInLine() : ctx.exp_mul().getStart().getCharPositionInLine();
                Error.printError(msj, line, col);
            }
    ...
    public T visitExp_mul(Exp_mulContext ctx) {
        ...
        if (ctx.exp_mul() != null) {
            ...
            if (op == '%') {
                ...
                if (msj != null) {
                    int line = (var1.getTipo() == Constants.DOUBLE || var1.getTipo() == Constants.STRING) ? ctx.exp_mul().getStart().getLine() : ctx.exp_pot().getStart().getLine(),
                            col = (var1.getTipo() == Constants.DOUBLE || var1.getTipo() == Constants.STRING) ? ctx.exp_mul().getStart().getCharPositionInLine() : ctx.exp_pot().getStart().getCharPositionInLine();
                    Error.printError(msj, line, col);
                }
                ...
                if (msj != null) {
                    int line = (var1.getTipo() == Constants.STRING) ? ctx.exp_mul().getStart().getLine() : ctx.exp_pot().getStart().getLine(),
                            col = (var1.getTipo() == Constants.STRING) ? ctx.exp_mul().getStart().getCharPositionInLine() : ctx.exp_pot().getStart().getCharPositionInLine();
                    Error.printError(msj, line, col);
                }
    ...
    public T visitExp_pot(Exp_potContext ctx) {
        ...
        if (ctx.exp_pot() != null) {
            ...
            if (var1.getTipo() == Constants.STRING || var2.getTipo() == Constants.STRING) {
                msj = Error.incompatibleData(Error.ERR_INT + ", " + Error.ERR_DOUBLE, Error.ERR_STRING);
                int line = (var1.getTipo() == Constants.STRING) ? ctx.exp_pot().getStart().getLine() : ctx.exp_una().getStart().getLine(),
                        col = (var1.getTipo() == Constants.STRING) ? ctx.exp_pot().getStart().getCharPositionInLine() : ctx.exp_una().getStart().getCharPositionInLine();
                Error.printError(msj, line, col);
            }
    ...
    public T visitExp_una(Exp_unaContext ctx) {
        if (ctx.OP_UNA() != null) {
            ...
            int line = ctx.exp_una().getStart().getLine(),
                    col = ctx.exp_una().getStart().getCharPositionInLine();
            if (op == '!') {
                if (var.getTipo() != Constants.INT) {
                    String msj = (var.getTipo() == Constants.STRING) ? Error.incompatibleData(Error.ERR_INT, Error.ERR_STRING) : Error.incompatibleData(Error.ERR_INT, Error.ERR_DOUBLE);
                    Error.printError(msj, line, col);
                }
    ...
    public T valueID(String id) {
        ArrayList<Hashtable<String, Object>> ts = tables;
        if (funcActual != null) {
            ts = funcActual.getTables();
        }
        for (Hashtable<String, Object> table : ts) {
