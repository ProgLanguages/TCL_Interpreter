...
//		result = (Variable)visitIdentificador(ctx.IDENTIFICADOR(), indice);
	...
	public T visitDeclaracion(DeclaracionContext ctx) {
		...
		if(temp == null){ // Si se cumple la variable no exist韆
...
// Error, se esperaba Constants.STRING
                ...
                if (var1.getTipo() == Constants.STRING); else if (var2.getTipo() == Constants.STRING); // Error, se esperaba tipo num茅rico, se encontr贸 Constants.STRING
...
//  Error, se encontr贸 Constants.STRING, se esperaba valor num茅rico
...
//  Error, se esperaba valor entero
                ...
                if (var1.getTipo() == Constants.STRING); else if (var2.getTipo() == Constants.STRING);
//  Error, se esperaba valor num茅rico se encontr贸 Constants.STRING
                var3 = new Variable(-1, null);
    ...
    public T visitExp_pot(Exp_potContext ctx) {
        ...
        if (ctx.exp_pot() != null) {
            ...
            if (var1.getTipo() == Constants.STRING); else if (var2.getTipo() == Constants.STRING);
//  Error se esperaba valor num茅rico
